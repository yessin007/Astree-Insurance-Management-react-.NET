...
export default function EditCompteAstree({ isOpen, onClose, onSave, currentItem }) {
    ...
    const [itemAstreeId, setItemAstreeId] = useState(currentItem.astreeId);
  ...
  const handleSave = () => {
    ...
    onSave({ numeroCompte:itemNumeroCompte,rib: itemRib,Iban:itemIban,banque:itemBanque, agence:itemAgence, adresse: itemAdresse,astreeId: itemAstreeId});
  }};
  const fetchAstree = () =>{
...
const handelAnnuler = () =>{
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Modifier le compte bancaire astree</ModalHeader>
        ...
        <ModalBody>
            ...
            <FormControl isRequired mt={4}>
                            <FormLabel >Astree</FormLabel>
                            <AsyncSelect
                              cacheOptions
                              defaultOptions
                              value={itemAstreeId}
                              getOptionLabel={e => e.matricule}
                              getOptionValue={e => e.id}
                              loadOptions={fetchAstree}
                              onInputChange={handleInputAstreeChange}
                              onChange={handleAstreeIdChange}
                              />
